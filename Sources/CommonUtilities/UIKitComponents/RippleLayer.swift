...
public class RippleLayer: CALayer, CAAnimationDelegate {
    ...
    public func stopEffectsImmediately() {
        ...
        effectIsRunning = false
        if rippleEnabled {
            if let rippleLayer = self.rippleLayer, let backgroundLayer = self.backgroundLayer {
    ...
    public func setRippleColor(color: UIColor, withRippleAlpha rippleAlpha: CGFloat = 0.3, withBackgroundAlpha backgroundAlpha: CGFloat = 0.3) {
        if let rippleLayer = self.rippleLayer, let backgroundLayer = self.backgroundLayer {
